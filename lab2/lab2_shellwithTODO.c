...
int main() {
    ...
    while (1) {
        ...
        strtok(cmdline, "\n");
        mulcmd_count = split_string(cmdline, ";", mulcommands);
        for(int j = 0; j < mulcmd_count; j++){
            cmd_count = split_string(mulcommands[j], "|", commands);
            if(cmd_count == 0) {
                continue;
                ...
                for(int i=0; i<cmd_count; i++) {
                    ...
                    int ret;
                    if(i != 0){
                        pipefd_2[0] = pipefd_1[0];
                        pipefd_2[1] = pipefd_1[1];
                    }//pipefd_2存储上一个管道信息
                    if(i != cmd_count - 1){
                        ret = pipe(pipefd_1);
                        if(ret < 0) {
